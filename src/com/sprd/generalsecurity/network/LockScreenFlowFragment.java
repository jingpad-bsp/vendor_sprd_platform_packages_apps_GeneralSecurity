...
public class LockScreenFlowFragment extends Fragment{
    ...
    private void updateUsageTotalData() {
        ...
        if (mDualSim) {
            mSim1FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data1),
                    Formatter.formatFileSize(mContext, mSim1UsageTotal)));
            // UNISOC: Bug706839 it doesn't show the value of Sim2UsageTotal when sim2 is primary
            mSim2FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data2),
                    Formatter.formatFileSize(mContext, mSim2UsageTotal)));
        } else {
            mSim1FlowTotalText.setText(String.format(getResources().getString(R.string.data_flow_data),
    ...
    class UidDetail implements Comparable<UidDetail>{
        ...
        public int compareTo(UidDetail another) {
            /**
             * UNISOC: Bug853122 app list not sort by data from largest to smallest in lock screen
             * @{
             */
            long result;
            result = another.usageSum - usageSum;
            if (result == 0) {
                return Collator.getInstance().compare(label.toString(), another.label.toString());
            } else if (result > 0) {
                return 1;
            } else {
                return -1;
    ...
    private UidDetail buildUidDetail(int uid) {
        ...
        switch (uid) {
            ...
            case TrafficStats.UID_TETHERING:
                /* UNISOC: Bug 676414 Attempt to write to field UidDetail.usageSim1' on a null object reference @{ */
