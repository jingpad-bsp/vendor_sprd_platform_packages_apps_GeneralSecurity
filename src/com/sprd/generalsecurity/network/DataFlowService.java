...
import com.sprd.generalsecurity.utils.NotificationChannelsUtil;
...
public class DataFlowService extends IntentService {
   ...
   // Query network usage since startTime to current time.
    ...
    /** UNISOC: add for bug1115913  @{ */
    private void notifyUser(String msg, int type, int count, int index, boolean isRemain) {
        Intent it = new Intent(REMIND_ACTION);
        it.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        it.putExtra(REMIND_TYPE, type);
        it.putExtra(WARNIN_SIM_COUNT, count);
        it.putExtra(WARNIN_SIM_INDEX, index);
        it.putExtra(WARNIN_IS_REMAIN,isRemain);
    /** @} */
        startActivity(it);
        ...
        // UNISOC: Bug1112691 When targetSdk >= 26, app must set the Channel
        NotificationChannelsUtil.createDefaultChannel(this);
        Notification notification = new Notification.Builder(this)
                ...
                .setChannelId(NotificationChannelsUtil.DEFAULT_CHANNEL)
                .build();
    ...
    private void checkDayDataAndNotify(int sim, INetworkStatsSession statsSession,
        ...
        if (dayRemindEnabled) {
            ...
            // UNISOC: Bug863804 when day restriction is zero, pop up notification
            if (remindDayDiffTrigger < 0 && dt.dayRestriction >= 0) {
                //trigger 1
                if (dataUsed > 0 && dataUsed >= dt.dayRestriction) {
                    String msgDaySIM = "";
                    if (simCount == COUNT_SINGLE) {
                    ...
                    // UNISOC: add for 1014151 the msg not translate when switch language
                    notifyUser(msgDaySIM, REMIND_DAY, simCount, sim, false);
    ...
    private void checkMonthDataRemainedAndNotify(int sim, INetworkStatsSession statsSession,
        ...
        if (monthRemindEnabled) {
            ...
            if (remindDiffTriggerWarn < 0 && dt.monthRestriction > 0 &&
                ...
                // UNISOC: add for 1014151 the msg not translate when switch language
                notifyUser(msg, REMIND_MONTH, simCount, sim, true);
                ...
                // UNISOC: add for 1014151 the msg not translate when switch language
                notifyUser(msg, REMIND_MONTH, simCount, sim, false);
